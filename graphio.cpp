...
std::string Graphio::input;

std::vector<std::vector<double>> Graphio::generate_gaussian_kernel(int radius, double sigma) {
	int size = 2 * radius + 1;
...
void Graphio::gaussian_blur(IMAGE* srcImg, IMAGE* dstImg, int radius, double sigma) {
...
void Graphio::set_text_style(LPCWSTR name, long height, unsigned char quality) {
	LOGFONT font;
	gettextstyle(&font);
...
void Graphio::solid_rectangle(COLORREF fill_color, int left, int top, int right, int bottom) {
	setfillcolor(fill_color);
	solidrectangle(left, top, right, bottom);
}
void Graphio::fill_round_rectangle(COLORREF fill_color, COLORREF line_color, int left, int top, int right, int bottom, int ellipsewidth, int ellipseheight) {
	setfillcolor(fill_color);
	setlinecolor(line_color);
	fillroundrect(left, top, right, bottom, ellipsewidth, ellipseheight);
}
void Graphio::output_line(COLORREF line_color, const std::vector<int>& position) {
	if (position.size() != 4) {
		return;
	}
	setlinecolor(line_color);
...
void Graphio::output_text(COLORREF text_color, int x, int y, const std::string& str) {
	settextcolor(text_color);
	outtextxy(x, y, string_to_lpcwstr(str));
}
void Graphio::draw_text(COLORREF text_color, const std::string& str, RECT* pRect, UINT uFormat) {
	settextcolor(text_color);
	drawtext(string_to_lpcwstr(str), pRect, uFormat);
}
void Graphio::input_text(const ExMessage& msg) {
